...
// ============================================================
// Search for "USERS" to find where needs to be modified! 
// ============================================================
...
void calcula(string positions, string input_file, vector<double> &v1, vector<double> &v2,
       ...
       bool IsSphere, vector<double> &v6, vector<double> &v7) {
  ...
  if(!Traks_file1) {
    cerr << "WARNING:  Failed to open file with optical detector positions"<< endl;
  }
  Traks_file1.seekg(0);
  vector<double> devx;
  vector<double> devy;
  ...
  std::cout<<"Number of PDS: "<<devx.size()<<std::endl;
  ...
  vector<double> v_hits, v_distance, v_rec_hits, v_offset_angle, v_d_center, v_x, v_devx;
  //loop over files
  ...
  for(int n=0; n!=tree->GetEntries(); ++n){
    ...
    for(int i=0; i<numberDevices; i++) {  
      ...
      // Modification requried from USERS:
      if (abs(devy.at(i)) > 219.99 && abs(devy.at(i)) < 220.01) continue;    // remove the other wall of laterals. 

      // ----------------  For laterals ---------------- 
      /*if (devy.at(i) < 740.) continue;    // remove cathode and far laterals
      if (abs(posSource[1] - devy.at(i)) < 57.)  continue; // remove the scintillation points outside the field cage. */
      // ----------------  End of laterals ---------------- 

      // ----------------  For cathodes ----------------  
      if (abs(devy.at(i)) > 740.) continue; // remove laterals       
      // ----------------  End of cathodes ---------------- 
    
      int entries = VUV_hits[i];
      if(entries < min_number_entries) continue; // Reminder: continue: skip the rest of the code when meeting the if-condition. 

      double distance_to_pd = sqrt( pow(posSource[0] - devx.at(i),2) +
                                    pow(posSource[1] - devy.at(i),2) +
                                    pow(posSource[2] - devz.at(i),2));

      // calculate theta
      double theta = -1;
      if (abs(devy.at(i)) > 740.) {
        // laterals
        double coseno = sqrt(pow(posSource[1] - devy.at(i),2))/distance_to_pd;
        theta = acos(coseno)*180/3.1416;
      }else {
        // cathode
        double coseno = sqrt(pow(posSource[0] - devx.at(i),2))/distance_to_pd;
        theta = acos(coseno)*180/3.1416;
      }
      ...
      if(IsRectangular) {
        ...
        if(IsSphere) {
          //------Semi-sphere--------- 
          // calculate solid angle    
          geo_factor = Omega_Dome_Model(distance_to_pd, theta);
        }
        else {
          //------Disk--------- 
          double d,h;
      ...
      double rec_N =  exp(-1.*distance_to_pd/L_abs)*gRandom->Poisson(num_phot_generated*geo_factor/(4*3.1416)); 

      //if (entries > 1000) cout << "OpDet: " << i << ", distance: " << distance_to_pd << ", theta: " << theta << ", G4: " << entries << ", S-A: " << rec_N << ", geo = " << geo_factor << endl;
      v_x.push_back(posSource[0]);
      v_devx.push_back(devx.at(i));
      v_hits.push_back(entries);
      v_distance.push_back(distance_to_pd);
      ...
      //if (devy.at(i) == Y && devz.at(i) == Z){
        //cout <<" PMT: " <<  i <<", dev x,y,z: "<<devx.at(i)<<", "<<devy.at(i)<<", "<<devz.at(i)<<", points: ("<<X<<", "<<Y<<", "<<Z<< "), Hits: " << entries << ", Geo: " << rec_N << ", ratio (Geant4/Geo/cos): "<<entries/rec_N*(cos(theta))<<" "<<exp(1.*distance_to_pd/L_abs)<<", Fractional differences: " << (entries-rec_N)/entries << ", Distance: " << distance_to_pd << ", Angle: " << theta << endl <<endl;
      //}
...
void Semi_Mode_Gen_v2() {
  ...
  // USERS path to the directory where the files are 
  string path_files = "/Users/s2138786/Downloads/Edinburgh/research/dune/light-sim/DUNE-vd/";
  // path + file containing the LDS id and positions (x, y, z)
  string positions = path_files + "optical_channel_mapping/dunevd_optical_mapping.txt";
  // path + file containing the Sim-Info
  string file_name = "semi_analytic_hits_vd_xenon_cathode_without_annode_ref"; //"semi_analytic_hits_vd_argon_lateral";
  string input_file = path_files + "new_traning_sample/" + file_name + ".root";

  TString save_dir = "./direct_plots_new/";
  ...
  double d_min = 0;
  double d_max = 2000.; // USERS changable 1400 - argon; 2000 -xenon
  double step_d = 50;
  
  //Center distance bins
  double range_d = 1100;//1100; 300;

  // Modification requried from USERS:
  const int M = 7;
  double range_d_array[M+1] = {0.,200.,400.,600.,800.,900.,1000.,range_d}; // cathode.
  //double range_d_array[M+1] = {0.,250.,400.,600.,800.,900.,range_d}; // lateral.
  //double range_d_array[M+1] = {0.,45.,100.,150.,200.,range_d}; // lateral. for interpolation method. 
  ...
  // *** Users *** must play and optimise the fit limits/options to guarantee
  ...
  for(int k=0; k < M; k++) { // M -- the bin to use for border effect. 
    ...
    for(int j=0; j < N; j++) { // N == 9
      ...
      // xenon
      if (j < 6) pars_ini[3] = -5000;
      else pars_ini[3] = -3000;
      // end of xenon cathode
      ...
      */
      ...
      GH[j][k]->SetParLimits(1,100.,200.);*/
      // end of argon cathode
      ...
      if (k == 4 && j == 8){
        GH[j][k]->SetParLimits(0,0.1,0.15);
        GH[j][k]->SetParLimits(1,199.,200.);
      }*/
  ...
  vector<double> v_distance, v_hits, v_rec_hits, v_offset_angle, v_d_center, v_x, v_devx;
...
// Canvas 0, the crowns plot, to study for border effect. 
  ...
  for(int k=0; k < M; k++) {
    ...
    if(hd_centers[k]->GetEntries()>0) {
      ...
      //title[k] =  "Distance to anode " +  to_string(int(hd_centers[k]->GetMean())) + "cm";
  ...
  for(int l=0; l < dim; l++){
    ...
    gg0[l]->GetYaxis()->SetRangeUser(0, 1.4); // USERS: changable. 
...
//------------------------------------------------------------------------------------------------------------------------------
  vector<double> vNmax[N], vdmax[N], vlambda[N], veNmax[N], vedmax[N], velambda[N], vd_center; // N -- 9. 
  ...
  for(int j=0; j < N; j++) { // N -- 9 
    ...
    for(int k=0; k < dim; k++){ // dim -- 4 
      ...
      if(j==0) {
        ...
        //cout<< "d_center.at(j)"<< d_center.at(j)<< endl;
      }
    }
  ...
  gf[0]->GetYaxis()->SetRangeUser(0,1.5);          // USERS: need to rewrite this
  gf[0]->GetXaxis()->SetRangeUser(0,range_d*1.05); // USERS: need to rewrite this to sset the range of border effect. 
  ...
  gf[1]->GetYaxis()->SetRangeUser(-100,1500);  // need to rewrite this
  ...
  TString fig_name3 = save_dir + file_name+"_border2.pdf";
  c->SaveAs(fig_name3);
  /////////////////////////////////////////////////////////////////////
